					    ...
					    const struct pci_device_id *pci_id)
{
	...
	if (snd_seq_device_new(card, 1, SNDRV_SEQ_DEV_ID_EMU10K1_SYNTH,
		...
		strlcpy(wave->name,"Emu-10k1 Synth",sizeof(wave->name));
		arg->hwptr = emu;
		arg->index = 1;
	...
	strlcpy(card->driver,emu->card_capabilities->driver,sizeof(card->driver));
	strlcpy(card->shortname,emu->card_capabilities->name,sizeof(card->shortname));
